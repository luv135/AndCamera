...
import com.luowei.cameralibrary.R;
...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	public void addFrameCallback(final IFrameCallback callback) {
		checkReleased();
		if (!mReleased && (callback != null)) {
			final CameraThread thread = mWeakThread.get();
			if (thread != null) {
				thread.mFrameCallback.add(callback);
			}
		}
	}

	public void removeFrameCallback(final IFrameCallback callback) {
		if (callback != null) {
			final CameraThread thread = mWeakThread.get();
			if (thread != null) {
				thread.mFrameCallback.remove(callback);
			}
		}
	}
	...
	static final class CameraThread extends Thread {
		...
		private final Set<IFrameCallback> mFrameCallback = new CopyOnWriteArraySet<IFrameCallback>();
		...
		public boolean setPreviewSize(){
			final UVCCamera camera = mUVCCamera;
			...
			int required_width = mWidth;
			int required_height = mHeight;
			...
			try {
				camera.setPreviewSize(preview_width, preview_height, UVCCamera.FRAME_FORMAT_MJPEG,mBandwidthFactor);
				...
				try {
					...
					camera.setPreviewSize(yuv_preview_width, yuv_preview_height, UVCCamera.FRAME_FORMAT_YUYV,mBandwidthFactor);
		...
		public void handleStopPreview() {
			...
			if (mIsPreviewing) {
				if (mUVCCamera != null) {
					if (DEBUG) Log.v(TAG_THREAD, "mUVCCamera.stopPreview()");
...
//				mUVCCamera.setFrameCallback(null, 0);
		...
		private final IFrameCallback mIFrameCallback = new IFrameCallback() {
			...
			public void onFrame(final ByteBuffer frame) {
				...
				for (final IFrameCallback callback: mFrameCallback) {
					try {
						callback.onFrame(frame);
//						Log.d(TAG,"mIFrameCallback");
					} catch (final Exception e) {
						mFrameCallback.remove(callback);
						Log.w(TAG, e);
					}
				}
			}
